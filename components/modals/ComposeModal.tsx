...
import { createTweetApi, uploadMediaApi, getCurrentUserProfile, UserProfile } from "@/utils/api";
...
export function ComposeModal({ onClose }: ComposeModalProps) {
  ...
  return (
    <>
      <div className="absolute inset-0 bg-[#080E1C]/50 backdrop-blur-[6px] z-50 transition-opacity duration-300" onClick={onClose} />
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-[420px] bg-white rounded-[24px] shadow-2xl z-[60] flex flex-col animate-in fade-in zoom-in-95 duration-200 border border-breezy-border-light">
        ...
        <div className="relative flex justify-between items-center px-4 py-3 border-b border-breezy-border-light h-14 bg-white shrink-0 rounded-t-[24px]">
          <button type="button" onClick={onClose} aria-label="Fermer" title="Fermer" className="p-1 hover:bg-gray-100 rounded-full transition-colors text-breezy-dark cursor-pointer border-none bg-transparent">
            <X size={20} />
          </button>
        ...
        <div className="p-4 flex gap-3 bg-white min-h-[140px] flex-col">
          ...
          {previewUrl && (
            <div className="relative w-full max-w-[300px] mt-1 ml-[52px]">
              <button type="button" onClick={() => { setSelectedFile(null); setPreviewUrl(null); }} className="absolute top-2 right-2 bg-black/60 text-white rounded-full p-1.5 cursor-pointer hover:bg-black transition-colors border-none" title="Supprimer l'image" aria-label="Supprimer l'image">
                <X size={16} />
              </button>
        ...
        <div className={`flex justify-between items-center px-4 py-3 border-t border-breezy-border-light bg-white shrink-0 ${!showPicker ? "rounded-b-[24px]" : ""}`}>
          <div className="flex items-center gap-2 text-breezy-blue relative">
            <input type="file" accept="image/*,video/*" className="hidden" ref={fileInputRef} aria-label="Ajouter un média" title="Ajouter un média" onChange={(e) => { const file = e.target.files?.[0]; if (file) { setSelectedFile(file); setPreviewUrl(URL.createObjectURL(file)); } e.target.value = ''; }} />
            
            <button type="button" onClick={() => fileInputRef.current?.click()} aria-label="Ajouter une image" title="Ajouter une image" className="hover:bg-blue-50 p-2 rounded-full transition cursor-pointer border-none bg-transparent flex">
              <Image size={18} />
            </button>

            <div className="relative">
              <button type="button" onClick={() => setShowTagMenu(!showTagMenu)} aria-label="Ajouter un tag" title="Ajouter un tag" className={`p-2 rounded-full transition cursor-pointer border-none flex ${showTagMenu ? "bg-blue-50 text-breezy-darkBlue" : "bg-transparent hover:bg-blue-50"}`}>
                <Tag size={18} />
              </button>
              ...
              {showTagMenu && (
                <div className="absolute bottom-[calc(100%+8px)] left-0 w-[160px] bg-white border border-breezy-border-light rounded-2xl shadow-[0_12px_40px_rgba(0,0,0,0.12)] z-[70] animate-in fade-in zoom-in-95 duration-100 flex flex-col py-1">
                  ...
                  {trendingTags.map((tag) => (
                    <button key={tag} type="button" onClick={() => addTagToText(tag)} className="text-left px-3 py-2 hover:bg-blue-50 text-[14px] font-semibold text-breezy-dark transition-colors cursor-pointer border-none bg-transparent">
                      #{tag}
                    </button>
                  ))}
                </div>
            ...
            <button type="button" onClick={() => setShowPicker(!showPicker)} aria-label="Ajouter un emoji" title="Ajouter un emoji" className={`p-2 rounded-full transition cursor-pointer border-none flex ${showPicker ? "bg-blue-50 text-breezy-darkBlue" : "bg-transparent hover:bg-blue-50"}`}>
              <Smile size={18} />
            </button>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button type="button" onClick={handlePublish} disabled={!canPublish} className="px-4 py-1.5 bg-breezy-blue hover:bg-breezy-darkBlue disabled:opacity-40 disabled:hover:bg-breezy-blue text-white font-bold text-[13px] rounded-full transition-all active:scale-[0.98] cursor-pointer disabled:cursor-not-allowed border-none">
        ...
        {showPicker && (
          <div ref={pickerRef} className="w-full border-t border-breezy-border-light bg-gray-50 animate-in slide-in-from-bottom duration-200 rounded-b-[24px] overflow-hidden">
            <EmojiPicker onEmojiClick={handleEmojiClick} autoFocusSearch={false} theme={Theme.LIGHT} width="100%" height={350} previewConfig={{ showPreview: false }} searchDisabled={false} skinTonesDisabled />
          </div>
        )}
      </div>
    </>
