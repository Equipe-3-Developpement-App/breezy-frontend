...
import { getTweetById, createCommentApi, likeTweetApi, retweetTweetApi } from "@/utils/api";
...
export default function CommentPage() {
  ...
  const handleLike = async (id: string, isParent: boolean, currentLiked: boolean, currentCount: number) => {
    ...
    if (isParent) {
      ...
      setComments((prev) =>
        prev.map((c) => (c.id === id ? { ...c, isLiked: nextLiked, likeCount: nextCount } : c))
      );
    }
  ...
  const handleRetweet = async (id: string, isParent: boolean, currentRetweeted: boolean, currentCount: number) => {
    ...
    if (isParent) {
      ...
      setComments((prev) =>
        prev.map((c) => (c.id === id ? { ...c, isRetweeted: nextRetweeted, retweetCount: nextCount } : c))
      );
    }
      ...
      <div className="flex-1 flex flex-col relative w-full h-full overflow-hidden select-none bg-breezy-bgLight rounded-[inherit]">
        ...
        <div className="flex-1 w-full overflow-y-auto pb-[180px] flex flex-col no-scrollbar">
          ...
          <div className="flex p-4 bg-breezy-bgLight border-b border-breezy-border-light text-left w-full shrink-0">
            <div className="w-[42px] h-[42px] shrink-0 rounded-full bg-gradient-to-br from-[#8F51D6] to-[#B62AC0] flex items-center justify-center font-bold text-white text-[16px]">
              {parentTweet.avatar || parentTweet.name.substring(0, 2).toUpperCase()}
            </div>
            <div className="flex flex-col ml-3 w-full">
              <div className="flex justify-between items-center w-full">
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className="font-inter font-bold text-[14.5px] text-breezy-dark">{parentTweet.name}</span>
                  <span className="font-inter text-[13px] text-breezy-gray">{parentTweet.handle}</span>
                </div>
              </div>
              <p className="font-inter text-[14.5px] text-breezy-dark mt-2 leading-[22px] whitespace-pre-line">{parentTweet.text}</p>
              <div className="border-b border-breezy-border-light py-2 text-[13px] text-breezy-gray font-medium font-inter">14:32 | 2 juin 2026</div>

              <div className="flex justify-between items-center text-breezy-gray text-[13px] font-medium max-w-[280px] mt-2 w-full">
                <div className="flex items-center gap-1.5"><MessageCircle size={17} /> <span>{parentTweet.commentCount}</span></div>
                <button type="button" aria-label="Retweeter" onClick={() => handleRetweet(parentTweet.id, true, parentTweet.isRetweeted, parentTweet.retweetCount)} className={`flex items-center gap-1.5 bg-transparent border-none p-0 cursor-pointer ${parentTweet.isRetweeted ? "text-green-500 font-bold" : "text-breezy-gray"}`}><Repeat2 size={17} /> <span>{parentTweet.retweetCount}</span></button>
                <button type="button" aria-label="Liker" onClick={() => handleLike(parentTweet.id, true, parentTweet.isLiked, parentTweet.likeCount)} className={`flex items-center gap-1.5 bg-transparent border-none p-0 cursor-pointer ${parentTweet.isLiked ? "text-[#E0526B] font-bold" : "text-breezy-gray"}`}><Heart size={17} className={parentTweet.isLiked ? "fill-[#E0526B]" : ""} /> <span>{parentTweet.likeCount}</span></button>
            ...
            {comments.length > 0 ? (
              comments.map((comment: any) => (
                <div key={comment.id} className="flex p-4 border-b border-breezy-border-light bg-breezy-bgLight w-full text-left">
                  <div className="w-[42px] h-[42px] shrink-0 rounded-full bg-gradient-to-br from-[#5194D6] to-[#2A2FC0] flex items-center justify-center font-bold text-white text-[15px]">
                    {comment.avatar || comment.name.substring(0, 2).toUpperCase()}
                  </div>
                  <div className="flex flex-col ml-3 w-full">
                    <div className="flex items-center gap-1.5">
                      <span className="font-inter font-bold text-[14.5px] text-breezy-dark">{comment.name}</span>
                      <span className="font-inter text-[13px] text-breezy-gray">{comment.handle}</span>
                      <span className="text-breezy-gray">·</span>
                      <span className="font-inter text-[13px] text-breezy-gray">{comment.time}</span>
                    </div>
                    <p className="font-inter text-[14.5px] text-breezy-dark mt-1 leading-[22px] whitespace-pre-line">{comment.text}</p>
        ...
        <div className="absolute bottom-[63px] left-0 right-0 bg-white border-t border-breezy-border-light p-3 z-30 flex flex-col gap-2">
          <form onSubmit={handleSubmitComment} className="flex gap-3 items-end">
            <div className="w-[36px] h-[36px] shrink-0 rounded-full bg-gradient-to-br from-[#5194D6] to-[#2A2FC0] flex items-center justify-center font-bold text-white text-[13px]">CR</div>
            <div className="flex-1 bg-gray-50 border border-gray-100 rounded-[20px] px-3 py-1.5 flex items-end gap-2">
              <textarea
                ...
                className="flex-1 bg-transparent resize-none outline-none font-inter text-[14.5px] text-breezy-dark max-h-[80px] pt-1"
              />
        ...
        {/* Fixed dynamic string property routing equation error */}
